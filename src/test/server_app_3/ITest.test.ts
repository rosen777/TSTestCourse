...
xdescribe("Server app integration test", () => {
  ...
  let token: string;
  it("should login new registered user", async () => {
    const result = await makeAwesomeRequest(
      {
        host: "localhost",
        port: 8080,
  ...
  it("should create and retrieve multiple reservations if authorized", async () => {
    await fetch(`http://localhost:8080/reservation/${createdReservationId}`, {
      method: HTTP_METHODS.POST,
      body: JSON.stringify(someReservation),
      headers: {
        authorization: token,
    ...
    expect(resultBody).toHaveLength(resultBody.length);
  });
